...
Function CatalogSpecs(setName)
  ...
  baseSpecs = Array( _
    Array("Номенклатура", "products", "1c_products.csv", "code"), _
  ...
  ElseIf setName = "all" Then
    CatalogSpecs = MergeArrays(MergeArrays(baseSpecs, nextSpecs), futureSpecs)
  Else
...
Function ExportCatalog(connection, outputDir, catalogName, objectType, sourceFile, mode)
  ...
  Dim isGroup, productGroupRef, productGroupCode, productGroupName
  ...
  stream.WriteLine "row_no;external_ref;code;name;deletion_mark;is_group;product_group_ref;product_group_code;product_group_name"
  ...
  Do While selection.Next()
    ...
    isGroup = SafeField(selection, "IsGroup")
    productGroupRef = SafeField(selection, "ProductGroupRef")
    productGroupCode = SafeField(selection, "ProductGroupCode")
    productGroupName = SafeField(selection, "ProductGroupName")

    stream.WriteLine Csv(CStr(rowNo)) & ";" & Csv(externalRef) & ";" & Csv(codeValue) & ";" & Csv(nameValue) & ";" & Csv(deletionMark) & ";" & Csv(isGroup) & ";" & Csv(productGroupRef) & ";" & Csv(productGroupCode) & ";" & Csv(productGroupName)
  Loop
...
Function BuildCatalogQuery(catalogName, mode)
  Dim refExpression, codeExpression, isGroupExpression
  Dim productGroupRefExpression, productGroupCodeExpression, productGroupNameExpression
  refExpression = """"""
  ...
  isGroupExpression = """"""
  productGroupRefExpression = """"""
  productGroupCodeExpression = """"""
  productGroupNameExpression = """"""

  If catalogName = "Номенклатура" Then
    isGroupExpression = "Элемент.ЭтоГруппа"
    productGroupRefExpression = "Элемент.Родитель"
    productGroupCodeExpression = "Элемент.Родитель.Код"
    productGroupNameExpression = "Элемент.Родитель.Наименование"
  End If
  ...
  BuildCatalogQuery = _
    ...
    "  " & isGroupExpression & " КАК IsGroup," & vbCrLf & _
    "  " & productGroupRefExpression & " КАК ProductGroupRef," & vbCrLf & _
    "  " & productGroupCodeExpression & " КАК ProductGroupCode," & vbCrLf & _
    "  " & productGroupNameExpression & " КАК ProductGroupName" & vbCrLf & _
    "ИЗ" & vbCrLf & _
...
Function SafeField(selection, fieldName)
  ...
  If Err.Number <> 0 Then
    ...
    Select Case fieldName
      ...
      Case "IsGroup"
        value = selection.IsGroup
      Case "ProductGroupRef"
        value = selection.ProductGroupRef
      Case "ProductGroupCode"
        value = selection.ProductGroupCode
      Case "ProductGroupName"
        value = selection.ProductGroupName
      Case Else
        value = ""
    End Select
  End If
...
Sub PrintUsage()
  ...
  WScript.Echo "  all  - base + next + future"
  WScript.Echo ""
