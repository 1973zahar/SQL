...
> Поточне робоче середовище CRM створюється як окрема PostgreSQL-база у VM `CRM-SQL` / Ubuntu `crm-sql`, а не як Docker-контейнер. Для цього сценарію використовуйте `docs/ubuntu-postgresql-runbook.md`. Цей документ лишається корисним для Docker Compose або локального dev-середовища.
